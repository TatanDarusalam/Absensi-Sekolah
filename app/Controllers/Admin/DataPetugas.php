...
class DataPetugas extends BaseController
{
   ...
   public function formEditPetugas($id)
   {
      ...
      $userSuperadmin = user()->toArray()['is_superadmin'];
      $petugasSuperadmin = $petugas['is_superadmin'];
      // Gateway untuk mengedit data petugas
      if ($userSuperadmin == '0' && ($petugasSuperadmin == '1' || $petugasSuperadmin == '0')) {
          session()->setFlashdata([
              'msg' => 'Anda tidak memiliki akses untuk mengedit data ini',
              'error' => true
          ]);
          return redirect()->to('/admin/petugas');
      }
      
      if (($userSuperadmin == '2' || $userSuperadmin == '3')) {
          session()->setFlashdata([
              'msg' => 'Anda tidak memiliki akses untuk mengedit data ini',
              'error' => true
          ]);
