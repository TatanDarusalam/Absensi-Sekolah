...
<script>
   ...
   function validasiKehadiran(idPresensi, validasi) {
      jQuery.ajax({
         url: "<?= base_url('/admin/absen-siswa/validasi-kehadiran'); ?>",
         type: 'post',
         data: {
            'id_presensi': idPresensi,
            'validasi': validasi
         ...
         success: function(response, status, xhr) {
            ...
            if (response['status']) {
               ...
               alert('Berhasil validasi kehadiran : ' + response['nama_siswa']);
            } else {
               alert('Gagal validasi kehadiran : ' + response['nama_siswa']);
            }
         },
         error: function(xhr, status, thrown) {
            console.log(thrown);
            alert('Gagal validasi kehadiran\n' + thrown);
         }
      });
   }
