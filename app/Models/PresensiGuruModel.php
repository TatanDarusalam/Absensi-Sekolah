...
class PresensiGuruModel extends PresensiBaseModel implements PresensiInterface
{
   ...
   public function getPresensiByTanggal($tanggal)
   {
      return $this->setTable('tb_guru')
         ->select('*')
         ->join(
            "(SELECT id_presensi, id_guru AS id_guru_presensi, tanggal, jam_masuk, jam_keluar, id_kehadiran, keterangan, validasi FROM tb_presensi_guru) tb_presensi_guru",
   ...
   public function updatePresensi($idPresensi = NULL, $idGuru, $tanggal, $idKehadiran, $jamMasuk = NULL, $jamKeluar = NULL, $keterangan = NULL, $validasi = 'off')
   {
