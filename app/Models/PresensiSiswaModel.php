...
class PresensiSiswaModel extends PresensiBaseModel implements PresensiInterface
{
   ...
   public function getPresensiByKelasTanggal($idKelas, $tanggal)
   {
      return $this->setTable('tb_siswa')
         ->select('*')
         ->join(
            "(SELECT id_presensi, id_siswa AS id_siswa_presensi, tanggal, jam_masuk, jam_keluar, id_kehadiran, keterangan, validasi FROM tb_presensi_siswa)tb_presensi_siswa",
   ...
   public function updatePresensi($idPresensi = NULL, $idSiswa, $idKelas, $tanggal, $idKehadiran, $jamMasuk = NULL, $jamKeluar, $keterangan = NULL, $validasi = 'off')
   {
      ...
      $data = [
         ...
         'validasi' => $validasi ?? $presensi['validasi'] ?? 'off'
      ];
